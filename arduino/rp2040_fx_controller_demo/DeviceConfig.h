...
static constexpr uint8_t kServoAMinAngle = 40;
static constexpr uint8_t kServoAMaxAngle = 80;
static constexpr uint8_t kServoBMinAngle = 100;
static constexpr uint8_t kServoBMaxAngle = 160;

// How often (ms) the claw advances one degree. Lower = faster sweep.
static constexpr uint16_t kClawStepIntervalMs = 15;

// When true, claw servos are de-energized (PWM off) whenever claw motion is
// not enabled. This reduces idle current draw and holding torque.
static constexpr bool kServoIdleWhenInactive = true;
